...
export async function updateUserAdminStatus(
  ...
  const pool = getDbPool();
  await pool.query(
    `UPDATE "user" SET is_system_admin = $1 WHERE user_id = $2;`,
    [isAdmin, userId],
  );
}
...
export async function updateUserDisplayName(
  ...
  await pool.query(
    `UPDATE "user" SET display_name = $1, fname = $2, lname = $3 WHERE user_id = $4;`,
...
export async function adminCreateUser(
  ...
  const result = await pool.query<{ user_id: number; created_at: string }>(
    ...
    `,
    [newUserId, email, fname, lname, displayName, passwordHash],
  );
...
export async function adminCreateClub(
  ...
  const result = await pool.query<{ club_id: number; created_at: string }>(
    `
    INSERT INTO "club" (club_id, owner_id, name, description, email, created_at, status, main_color, icon_name)
  ...
  await pool.query(
    `
    INSERT INTO membership (membership_id, user_id, club_id, membership_role)
    VALUES ((SELECT COALESCE(MAX(membership_id),0)+1 FROM membership), $1, $2, 'board_member')
    ON CONFLICT (user_id, club_id) DO UPDATE SET membership_role = EXCLUDED.membership_role;
    `,
...
export async function adminDeleteClub(clubId: number): Promise<void> {
  ...
  await pool.query(`DELETE FROM "joinrequest" WHERE target_club_id = $1`, [
    clubId,
  ]);
